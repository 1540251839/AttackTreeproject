...
class EquipmentMessage:
    ...
    def __init__(self):
        ...
        self.Router_suggestions = [i.strip() for i in """
                ...
                在物理安全性方面，该装置同样存在重大的安全隐患，特别是对USB接口的攻击途径。因此，为了防止通过USB接口发起的攻击，该装置需要采取额外的物理安全措施，例如限制对USB端口的访问或者完全封闭这些端口。
        ...
        self.Switch_suggestions = [i.strip() for i in """
                ...
                该装置还可能面临DDoS攻击。为防止此类攻击，网络管理员应配置交换机的流量控制功能，限制特定端口上的流量速率，从而减少DDoS攻击的风险。同时，可以配置交换机的访问控制列表（ACL），过滤掉恶意流量，提高网络的安全性。
                ...
                最后，该装置的物理安全也至关重要。为防范物理安全威胁，网络管理员应确保交换机放置在安全的环境中，限制对交换机的物理访问，并定期检查交换机的配置和状态，以确保其正常运行。
               """.split("\n\n")]
        ...
        self.Terminal_suggestions = [i.strip() for i in """
                ...
                不安全的配置也会增加该装置的安全风险。用户应确保网络连接加密，并使用安全的Wi-Fi密码。

                该装置的物理安全威胁也需要引起重视。用户应限制对设备的物理访问，并采取措施防止数据被盗或设备损坏。
        ...
        self.Admin_suggestions = [i.strip() for i in """
                ...
                未授权访问也是该装置一个主要的安全风险。管理员应启用访问控制机制，如使用强密码和多因素认证，以及限制对管理终端的访问权限。

                中间人攻击是该装置另一种潜在的威胁。为防止此类攻击，管理员应确保管理终端与网络设备之间的通信使用加密技术，并定期检查网络配置，确保没有开放端口或不当的安全设置。

                权限提升攻击也是该装置一个常见的威胁。为防止此类攻击，管理员应确保系统中的权限设置合理，并定期检查系统日志，以发现异常行为。

                该装置的物理安全威胁也需要引起重视。管理员应确保管理终端的物理位置安全，限制对设备的物理访问，并采取措施防止设备被盗或被物理破坏。

                该装置的数据泄露也是一个严重的安全风险。管理员应确保管理终端存储的敏感信息得到有效的加密保护，并采取措施防止数据泄露。

                网络钓鱼和社交工程是另一种攻击方式。管理员应提高警惕，不轻易泄露个人信息，不随意打开未知来源的电子邮件附件，以及访问受信任的网站。
                """.split("\n\n")]
    ...
    def _generate_equipment_name(self):
        ...
        # 管理终端节点列表及其对应的具体设备型号
        admin_terminal_models = ["Lenovo ThinkPad X1 Carbon"]
        admin_terminal_node = [13]
    ...
    def _generate_equipment_status(self):
        ...
        for ID in node_id:
            self.equipment_status[ID] = f"Running ({random.randint(0, 10 + int(len(node_id)/10))} days ago)"

    def _generate_equipment_suggestion(self):
        node_id = list(self.equipment_type.keys())

        for ID in node_id:
            self.equipment_suggestion[ID] = f"Running ({random.randint(0, len(node_id))} days ago)"
